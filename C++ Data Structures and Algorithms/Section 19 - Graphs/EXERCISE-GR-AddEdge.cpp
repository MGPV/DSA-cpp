...
class Graph {
    ...
    public:
        ...
        bool addEdge(string vertex1, string vertex2){
            if(adjList.count(vertex1) != 0 && adjList.count(vertex2) != 0){ //check that vertices exist
                adjList.at(vertex1).insert(vertex2); //inserts A edge
                adjList.at(vertex2).insert(vertex1); //inserts B edge
                return true;
            }
            return false;
        }
