...
class HashTable {
    ...
    public:
        ...
        int hash(string key){
            int hash = 0; //initiate hash
            for(int i = 0; i < key.length(); i++){ //for loop that ultimately makes hash = to a number that is within the size limits of the table
                int asciiValue = int(key[i]); 
                hash = (hash + asciiValue * 23) % SIZE;
            }
            return hash; //return the hash number
        }
